...
function readInputFile(inputFile::String)
    ...
    nb_lines = parse(Int64,data[1])


    # For each line of the input file
    Game = Matrix{Int64}(undef, nb_lines, nb_lines)
    i = 1
    for line in data[2:2+nb_lines-1]

        lineSplit = split(line, ",")

        for j in 1:nb_lines 
            Game[i,j] = parse(Int64,lineSplit[j])
        end
    ...
    N = zeros(Int64, nb_lines*nb_lines, nb_lines*nb_lines)
    ...
    for i in 1:nb_lines
        ...
        for j in 1:nb_lines
            ...
            num_case = (i-1)*nb_lines + j


            if Game[i,j] == 0                           # flèche vers le haut
                                     
                k = 1
                while num_case -k*nb_lines >=1
                    N[num_case,num_case-k*nb_lines] = 1     #on marque tous les potentiels successeurs de la case num_case
                    k+=1
                end
                


            elseif Game[i,j] == 1                           # flèche vers le haut à droite
                
                for k in 1:min(nb_lines-j, i-1)
                    N[num_case, num_case - k*(nb_lines-1)] = 1
                end



            elseif Game[i,j] == 2                           # flèche vers la droite

                for k in 1:nb_lines - num_case%nb_lines
                    a = num_case
                    b = num_case + k 
                    N[a,b] = 1
                end
            ...
            elseif Game[i,j] == 3                           # flèche vers le bas à droite

                for k in 1:min(nb_lines-j, nb_lines-i)
                    N[num_case, num_case + k*(nb_lines+1)] = 1
                end
                


            elseif Game[i,j] == 4                           #flèche vers le bas

                k = 1
                while  num_case+k*nb_lines <= nb_lines*nb_lines
                    N[num_case, num_case+k*nb_lines] = 1
                    k+=1
                end



            elseif Game[i,j] == 5                           #flèche vers le bas à gauche

                for k in 1:min(nb_lines-i, j-1)
                    N[num_case, num_case + k*(nb_lines-1)] = 1
                end               



            elseif Game[i,j] == 6                           #flèche vers la gauche

                for k in 1:((num_case-1)%nb_lines)
                    a = num_case
                    b = num_case - k 
                    N[a,b] = 1
                end
            ...
            elseif Game[i,j] == 7                           #flèche vers le haut à gauche

                for k in 1:min(j-1, i-1)
                    N[num_case, num_case - k*(nb_lines+1)] = 1
                end            
    ...
    Constraint_Matix = Matrix{Int64}(undef,nb_lines,nb_lines)
    row=1
    for line in data[2+nb_lines+1:2*(nb_lines+1)]
        lineSplit = split(line, ",")

        for col in 1:nb_lines

            Constraint_Matix[row,col] = parse(Int64,lineSplit[col])
    ...
    return Game, N, Constraint_Matix
end
